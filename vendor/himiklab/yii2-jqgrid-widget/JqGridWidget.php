...
//     	JqGridAsset::register();
    ...
    protected function processingGridSettings($gridUserSettings)
    {
        ...
        $gridSettings['url'] = $this->requestUrl . $this->prefix_url_param . 'action=request';
        ...
        if ($this->enableCellEdit) {
            ...
            $gridSettings['cellurl'] = $this->requestUrl . $this->prefix_url_param . 'action=edit';
        }
    ...
    protected function processingPagerSettings($pagerUserSettings)
    {
        ...
        foreach ($pagerUserSettings as $optionName => $optionSettings) {
            ...
            switch ($optionName) {
                case 'edit':
                    $editSettings['url'] = $this->requestUrl . $this->prefix_url_param . 'action=edit';
                    $pagerOptions['edit'] = array_merge($editSettings, $optionSettings);
                    break;
                case 'add':
                    $addSettings['url'] = $this->requestUrl . $this->prefix_url_param . 'action=add';
                    $pagerOptions['add'] = array_merge($addSettings, $optionSettings);
                    break;
                case 'del':
                    $delSettings['url'] = $this->requestUrl . $this->prefix_url_param . 'action=del';
